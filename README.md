...
This site can be previewed with Docker so Ruby gems stay inside Docker instead of being installed on the host.
...
```sh
docker compose run --rm site bash -lc "bundle install && bundle exec jekyll build"
```
